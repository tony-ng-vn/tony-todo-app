...
- **Primary navigation, 6 of 8 surfaces:** Focus (Flow view), Inbox (medium-confidence review tray with Accept/Dismiss/Snooze), Waiting, History (dismissed loops, with Restore), Meetings (loops grouped by source meeting), Settings (connector sync status, recent AI activity). All six are wired to real data and were manually verified end to end in the browser against the live backend, not just unit tested.
- **Real drafting (Tier 1 autonomy).** The Waiting view's "Draft follow-up" generates a genuine, contextual message via an LLM call, ownership-isolated per user, text-only (never sends anything). Verified live against a real loop.
- **Audit log (FR-15).** Every loop creation and draft generation writes a durable `audit_log` entry, surfaced in Settings under "Recent AI activity."
- **Deliberately not built yet:** Routines, Chat, and any Tier 2+ autonomy (auto-resolving loops from observed replies, actually sending mail). Routines/Chat are explicitly Phase 3 in this PRD's own sequencing (Section 41). Tier 2+ requires Gmail/Calendar to exist first (see below).

### Still blocked on external action (not resolvable by an agent)

- **Gmail + Calendar ingestion.** Needs a Google Cloud OAuth app. **Correction to earlier guidance in this section:** the "weeks-long CASA security assessment" framing only applies if this app will serve users beyond a small named "Testing" roster. For personal, single-user use, Google requires no verification at all -- see `docs/google-oauth-setup.md` for the exact steps. Generic OAuth token-lifecycle logic and an `oauth_connections` schema are already built and tested (provider-agnostic); the actual Gmail/Calendar API integration code is deliberately not written yet, because it can't be verified against Google's real API without real credentials, and shipping untested integration code would be a worse outcome than waiting.
